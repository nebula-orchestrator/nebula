...
def stop_container(container_name, stop_timout=5):
    ...
    try:
        return cli.stop(container_name, stop_timout)
    except:
        try:
...
# create host_config
def create_container_host_config(port_binds):
    try:
...
def run_container(app_name, container_name, image_name, bind_port, ports, env_vars, version_tag="latest", docker_registry_user="", docker_registry_pass=""):
    create_container(app_name, container_name, image_name + ":" + version_tag, create_container_host_config(bind_port), ports, env_vars)
