...
| CLI | WIP | yes | yes | yes |
| GUI | WIP | yes | yes | no |
| Load Balancing | yes- supports bringing your own HAProxy\Nginx\etc in a 2 step process (the first between instances & the 2nd between containers on that instance) | HTTP only for outside connections - marathon-lb | yes | partial - auto routes inside the cluster but you still need to LB between cluster nodes from the outside  |
